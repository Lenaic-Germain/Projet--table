...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['delete_cow'])) {
    ...
    if ($row['total'] == 0) {
        $conn->query("ALTER TABLE cows AUTO_INCREMENT = 1");
    }
...
<section>
    ...
    <table border="1">
        ...
        <?php while ($row = $result->fetch_assoc()): ?>
            <tr>
                ...
                <td><?php echo htmlspecialchars($row["rfid_tag"] ?? 'Not Assigned'); ?></td>
                <td><?php echo htmlspecialchars($row["milk_amount"] ?? 'N/A'); ?></td>
    ...
    <table border="1">
        ...
        <?php
        $log_result = $conn->query("SELECT * FROM cow_logs ORDER BY log_time DESC LIMIT 10");
        while ($log = $log_result->fetch_assoc()):
        ?>
            <tr>
                <td><?php echo htmlspecialchars($log["log_id"]); ?></td>
                <td><?php echo htmlspecialchars($log["cow_id"]); ?></td>
